	...
	$msg = "";
		...
		if(isset($_POST['submit'])) {
			...
			if($guest != "" && $email && $phone != "" && $date_res != "" && $time != "" && $suggestions != "") {
				...
				$check = $con->query("SELECT * FROM tblreservation WHERE no_of_guest='".$guest."' AND email='".$email."' AND phone='".$phone."' AND date_res='".$date_res."' AND time='".$time."' LIMIT 1");
				...
				if($check->num_rows) {
					...
					$insert = $con->query("INSERT INTO tblreservation(no_of_guest, email, phone, date_res, time, suggestions) VALUES('".$guest."', '".$email."', '".$phone."', '".$date_res."', '".$time."', '".$suggestions."')");
